...
def test_app_flow() -> None:
    def check_for_user_login(
        email: str, attempts: int = 60, sleep_seconds: int = 1
    ) -> bool:
        user_logged_in = False
        attempt_count = 0

        click.echo(
            "Waiting for log in... " f"(You have {attempts * sleep_seconds} seconds)"
        )
        ...
        while not user_logged_in and attempt_count < attempts:
            ...
            token_expired = dt.datetime.now() > dt.datetime.fromisoformat(
                user.token_expires
            )
    ...
    while add_new_user:
        ...
        encoded_email = base64.urlsafe_b64encode(new_user_email.encode()).decode()

        click.echo(
            f"Please send this link to your friend to log in: {login_url + '/' + encoded_email}"
        )
